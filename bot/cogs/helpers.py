...
cluster = MongoClient("mongodb+srv://dbBot:samarth1709@cluster0.moyjp.mongodb.net/myFirstDatabase?retryWrites=true&w=majority")
...
def toTime(x : str):
    ...
    elif x.endswith('hr') : return float(x[:-2])*3600
    elif x.endswith('h') : return float(x[:-1])*3600
    elif x.endswith('m') : return float(x[:-1])*60
    elif x.endswith('min') : return float(x[:-3])*60
...
async def showBagitems(ctx):
    ...
    collection = db['accounts']
    accounts = collection.find_one({'_id' : 1})
    if not str(ctx.author.id) in accounts.keys(): return await ctx.send("You don't own any items :eyes:. But some and use the command again")
    if not 'bag' in accounts[str(ctx.author.id)].keys() : return await ctx.send("You don't own any items :eyes:. But some and use the command again")
...
async def sellItem(ctx , item , amount = 1):
    ...
    for thing in shop:
        if thing['name'] == item.lower():
            ...
            if accounts[str(ctx.author.id)]['bag'][item.lower()] == 0: del accounts[str(ctx.author.id)]['bag'][item.lower()]
            ...
            await ctx.send(f"You successfully sold `{amount}` {item.lower()} and earned `{int(thing['price']*0.6*amount)}`")