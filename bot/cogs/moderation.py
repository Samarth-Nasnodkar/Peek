...
class Moderation(commands.Cog):
    ...
    async def mute(self , ctx , member : discord.Member , duration = None):
        try:
            if (ctx.author.guild_permissions.administrator or ctx.author.guild_permissions.manage_roles) and (ctx.author.top_role > member.top_role):
                for role in ctx.guild.roles:
                    if role.name.lower() == "muted" : 
                        ...
                        if isinstance(duration , int) or isinstance(duration , float) : await asyncio.sleep(duration)
                        elif isinstance(duration , str) : await asyncio.sleep(toTime(duration))
    ...
    async def prefix(self , ctx , prefix = None):
        ...
        done = update_prefix(self.client , prefix , ctx.guild.id)
        if done:
    ...
    async def temprole(self , ctx , user : discord.Member , role : discord.Role , duration = None):
        if ctx.author.guild_permissions.manage_roles and ctx.author.top_role > user.top_role and role < ctx.author.top_role and timeConvertible(duration):
            await user.add_roles(role)
